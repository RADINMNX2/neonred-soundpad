...
import { X, Eye, Palette, Zap, Waves, LayoutGrid, Maximize2, Hash, Check, ChevronDown, Maximize } from 'lucide-react';
...
// --- CONSTANTS ---
const PRESET_COLORS = [
  '#ef4444', '#ec4899', '#d946ef', '#8b5cf6', '#3b82f6', 
  '#06b6d4', '#10b981', '#eab308', '#f97316', '#ffffff',
];
    ...
    <div className="relative" ref={containerRef}>
       <button onClick={() => setIsOpen(!isOpen)} className="w-full flex items-center justify-between p-2 bg-black/40 border border-white/10 rounded-xl hover:border-pink-500/50 hover:bg-black/60 transition-all group">
          <div className="flex items-center gap-3">
             <div className="w-10 h-10 rounded-lg shadow-lg border border-white/10" style={{ backgroundColor: color, boxShadow: `0 0 15px ${color}40` }}></div>
         ...
         <div className="absolute top-full left-0 right-0 mt-3 p-4 bg-zinc-900 border border-white/10 rounded-2xl shadow-2xl shadow-black/80 z-50 animate-slide-up origin-top">
            <div className="mb-6">
                 <div className="relative h-4 w-full rounded-full ring-1 ring-white/10">
                     ...
                     <input type="range" min="0" max="360" value={hue} onChange={handleSpectrumChange} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" />
                     <div className="absolute top-1/2 -translate-y-1/2 w-5 h-5 rounded-full bg-white shadow-[0_0_10px_rgba(0,0,0,0.5)] border-2 border-white pointer-events-none transition-transform duration-75" style={{ left: `calc(${(hue / 360) * 100}% - 10px)` }}></div>
                 </div>
            </div>
            <div className="grid grid-cols-5 gap-2">
               {PRESET_COLORS.map((c) => (
                 <button key={c} onClick={() => { onChange(c); setHexInput(c); }} className={`w-8 h-8 rounded-full border border-white/10 transition-all hover:scale-110 relative group/color ${color.toLowerCase() === c.toLowerCase() ? 'ring-2 ring-white' : ''}`} style={{ backgroundColor: c }}>
                    {color.toLowerCase() === c.toLowerCase() && <Check size={12} className="text-black/50 absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 font-bold" />}
                 </button>
               ))}
            </div>
         </div>
...
const PlayerSettingsModal: React.FC<PlayerSettingsModalProps> = ({
  ...
  const handleVisChange = (key: keyof VisualizerConfig, value: any) => {
    onVisUpdate({ ...visConfig, [key]: value });
  };
  ...
  return (
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      {/* Main Container */}
      <div className="relative w-full max-w-4xl bg-zinc-950 border border-pink-500/20 rounded-[3rem] shadow-2xl shadow-pink-900/20 animate-slide-up flex flex-col max-h-[90vh] overflow-hidden">
        
        {/* Animated Background Decor */}
        <div className="absolute -top-24 -right-24 w-80 h-80 bg-pink-600/10 blur-[120px] pointer-events-none"></div>
        <div className="absolute -bottom-24 -left-24 w-80 h-80 bg-purple-600/10 blur-[120px] pointer-events-none"></div>

        {/* Header */}
        <div className="relative p-8 border-b border-white/5 bg-zinc-900/40 z-10 rounded-t-[3rem] flex justify-between items-center">
        ...
        {/* Content Body */}
        <div className="flex-1 overflow-y-auto p-8 custom-scrollbar relative z-10 bg-zinc-900/30">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 animate-fade-in">
                <div className="space-y-8">
                    ...
                    <div className="flex items-center justify-between bg-zinc-900/50 p-6 rounded-[2rem] border border-white/5 shadow-inner">
                        <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${visConfig.isEnabled ? 'bg-green-500/20 text-green-400 shadow-lg shadow-green-500/5' : 'bg-zinc-800 text-zinc-600'}`}>
                        ...
                        <button onClick={() => handleVisChange('isEnabled', !visConfig.isEnabled)} className={`w-16 h-9 rounded-full p-1.5 transition-all duration-500 ${visConfig.isEnabled ? 'bg-pink-600 shadow-[0_0_20px_rgba(236,72,153,0.4)]' : 'bg-zinc-800'}`}>
                            <div className={`w-6 h-6 rounded-full bg-white shadow-xl transform transition-transform duration-500 ${visConfig.isEnabled ? (isRTL ? '-translate-x-7' : 'translate-x-7') : 'translate-x-0'}`}></div>
                        </button>
                    </div>

                    <div className={`space-y-8 transition-all duration-700 ${visConfig.isEnabled ? 'opacity-100 scale-100' : 'opacity-30 scale-95 pointer-events-none grayscale'}`}>
                        <ControlGroup title="Physical Geometry">
                            <CustomSlider label="Visual Scale" icon={Maximize2} min={0.5} max={2.0} step={0.1} value={visConfig.height} onChange={(v: number) => handleVisChange('height', v)} unit="x" />
                            <CustomSlider label="Bar Density" icon={LayoutGrid} min={20} max={100} step={2} value={visConfig.barCount} onChange={(v: number) => handleVisChange('barCount', v)} />
                        ...
                        <ControlGroup title="Response & Aesthetics">
                            <CustomSlider label="Audio Sensitivity" icon={Waves} min={0.5} max={3.0} step={0.1} value={visConfig.sensitivity} onChange={(v: number) => handleVisChange('sensitivity', v)} unit="x" />
                            <div className="space-y-4">
                                ...
                                <div className="flex p-1 bg-black/40 rounded-2xl border border-white/5">
                                    <button onClick={() => handleVisChange('colorMode', 'auto')} className={`flex-1 py-3 text-xs font-bold rounded-xl transition-all ${visConfig.colorMode === 'auto' ? 'bg-pink-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}>Adaptive (Cover)</button>
                                    <button onClick={() => handleVisChange('colorMode', 'manual')} className={`flex-1 py-3 text-xs font-bold rounded-xl transition-all ${visConfig.colorMode === 'manual' ? 'bg-pink-600 text-white shadow-lg' : 'text-zinc-500 hover:text-zinc-300'}`}>Fixed Neon</button>
                                </div>
                                {visConfig.colorMode === 'manual' && <div className="pt-2 animate-slide-up"><NeonColorPicker color={visConfig.manualColor} onChange={(c) => handleVisChange('manualColor', c)} /></div>}
                            </div>
                        </ControlGroup>
                    </div>
                </div>
                ...
                <div className="flex flex-col h-full space-y-6">
                        <ControlGroup title="Preview">
                            <div className="relative w-full h-[300px] bg-black rounded-[2.5rem] border border-white/10 overflow-hidden shadow-2xl flex items-center justify-center p-8">
